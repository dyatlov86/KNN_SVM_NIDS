...
for i in range(7,k+2,2):
    ...
    y_pred = cross_val_predict(knn, X_packets, y_packets, cv=8)
    f1scores = cross_val_score(knn, X_packets, y_packets, cv=8,scoring="f1_weighted",n_jobs=4)
    acscores = cross_val_score(knn, X_packets, y_packets, cv=8,scoring="accuracy",n_jobs=4)
    recscores = cross_val_score(knn, X_packets, y_packets, cv=8,scoring="recall_weighted",n_jobs=4)
    prescores = cross_val_score(knn, X_packets, y_packets, cv=8,scoring="precision_weighted",n_jobs=4)
    cm = confusion_matrix(y_packets, y_pred)
    cm_df = pd.DataFrame(cm,index =["asreproasting","ddos","exploit-samba","kerberos-enum","nmap","samba-enum"] , columns = ["asreproasting","ddos","exploit-samba","kerberos-enum","nmap","samba-enum"])
    print(cm_df)
