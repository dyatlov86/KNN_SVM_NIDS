...
packets = pd.read_csv('binary.csv')
...
exploit = pd.read_csv('normal.csv')
#X = data.iloc[:, :-1].values
#y = data.iloc[:, -1].values
packets.head()

X_packets = exploit[['ip.ttl', 'tcp.window_size', 'tcp.ack', 'tcp.seq', 'tcp.stream','frame.time_relative', 'tcp.time_relative', 'tcp.port']] #setting the col names
y_packets = exploit['label'] #setting the col names


file1 = open('normal.csv', 'r')
count = 0
