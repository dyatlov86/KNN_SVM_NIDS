...
for i in range(21,k+2,2):
    print(str(i)+"\n")
    knn = KNeighborsClassifier(n_neighbors=i)
    knn.fit(X_train, y_train)
    y_pred = cross_val_predict(knn, X_packets, y_packets, cv=10)
    f1scores = cross_val_score(knn, X_packets, y_packets, cv=10,scoring="f1_macro",n_jobs=4)
    acscores = cross_val_score(knn, X_packets, y_packets, cv=10,scoring="accuracy",n_jobs=4)
    recscores = cross_val_score(knn, X_packets, y_packets, cv=10,scoring="recall_macro",n_jobs=4)
    prescores = cross_val_score(knn, X_packets, y_packets, cv=10,scoring="precision_macro",n_jobs=4)
