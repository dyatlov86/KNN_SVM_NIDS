...
for i in range(10,k+2,1):
    ...
    f1scores = cross_val_score(svm_model, X_packets, y_packets, cv=10,scoring="f1_weighted",n_jobs=5)
    acscores = cross_val_score(svm_model, X_packets, y_packets, cv=10,scoring="accuracy",n_jobs=5)
    recscores = cross_val_score(svm_model, X_packets, y_packets, cv=10,scoring="recall_weighted",n_jobs=5)
    prescores = cross_val_score(svm_model, X_packets, y_packets, cv=10,scoring="precision_weighted",n_jobs=5)
